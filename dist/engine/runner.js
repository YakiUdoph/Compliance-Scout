...
export class BatchComplianceRunner {
     ...
     */
    async runJobAsync(jobId, businesses) {
        try {
            ...
            try {
                ...
                console.warn(`[Runner] Non-fatal error writing report files: ${err.message}`);
            }
            markJobCompleted(jobId, summary);
            return summary;
        }
        catch (error) {
            const errorMsg = error.message || 'Batch orchestrator execution failed.';
     ...
     */
    async runBatch(businesses, onProgress) {
            ...
            return limit(async () => {
                try {
                    ...
                    const failedResult = {
                        ...
                        amountOwed: null,
                        summaryNote: `Execution error: ${err.message}`,
                        screenshotPath: '',
                        certPdfPath: null,
                        filingUrl: null,
                        stepCount: 0,
