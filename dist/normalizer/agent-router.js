import { NormalizationOutputSchema } from './schema.js';
export class AgentRouterNormalizer {
     ...
     */
    async normalizeStatus(rawStatusText, stateCode, rawAmountOwedText) {
...
Respond ONLY with a valid JSON object matching this TypeScript schema:
{
  ...
  "summaryNote": "A concise 1-sentence legal summary of entity standing in ${stateCode}"
                ...
                console.warn(`[AgentRouterNormalizer] HTTP request to agentrouter.org failed, falling back to rule engine: ${error.message}`);
            }
        }
    ...
    fallbackRuleNormalize(rawStatusText, stateCode, rawAmountText) {
        const textLower = rawStatusText.toLowerCase();
        let status = 'GOOD_STANDING';
        let amountOwed = rawAmountText || null;
